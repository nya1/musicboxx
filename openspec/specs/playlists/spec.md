...
Playlists organize songs in Musicboxx: a default **Favorites** list, user-created playlists, optional **nesting** (parent/child), subtree views with deduplication, **ancestor links** on nested playlist detail, **per-playlist accent colors**, a **Playlists** overview **overflow menu** (rename, default playlist, nested create, move, change color, delete), **user-configurable default playlist** for new songs, and local persistence in the browser.
...
- **AND** its playlist record includes a persisted accent color
...
- **AND** the chosen color SHALL be drawn from the product’s curated palette (e.g. uniformly at random when the playlist is created)
