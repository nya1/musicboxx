...
Define **local full-text search** over the user’s saved songs (IndexedDB) so the Library home can filter and rank tracks by relevance using **BM25-style** retrieval, without sending catalog text to a server.
